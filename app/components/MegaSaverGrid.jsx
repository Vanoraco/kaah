...
export function MegaSaverGrid({ megaSaverItems, megaSaverBanner, showViewMoreButton = true }) {
  const [isVisible, setIsVisible] = useState(false);
  const sectionRef = useRef(null);

  // Debug metaobject data on mount
  useEffect(() => {

    debugMetaobjects(megaSaverItems);
  }, [megaSaverItems]);
  ...
  function processItemsData(itemsData) {
    return itemsData.nodes.map((item) => {
        ...
        item.fields.forEach(field => {
          if (field && field.key) {
            ...
            if ((field.key === 'image' || field.key.includes('image_')) && field.reference && field.reference.image) {
              ...
              console.log(`Found image reference for field ${field.key}:`, imageUrl);
            }
            ...
            if (field.key === 'product' && field.reference) {
              ...
              if (!imageUrl && field.reference.featuredImage) {
                ...
                console.log('Using product featured image:', imageUrl);
              }
            }
          }
        });
      ...
      // Debug price values
      console.log('Price values for item:', {
        itemId: item.id,
        itemHandle: item.handle,
        price_: fieldMap.price_,
        price: fieldMap.price,
        productPrice: productRef ? productRef.priceRange?.minVariantPrice?.amount : 'No product price'
      });
      ...
      if (fieldMap.price_ !== undefined && fieldMap.price_ !== null && fieldMap.price_ !== '') {
        ...
        console.log(`Using metaobject price_: ${price} for item ${item.handle}`);
      }
      // Fallback to old price field for backward compatibility
        ...
        price = fieldMap.price;
        console.log(`Using metaobject price (legacy): ${price} for item ${item.handle}`);
      }
      // Last resort: use product price
        ...
        console.log(`Using product price: ${price} for item ${item.handle}`);
      }
  ...
  return (
    ...
    >
      ...
      {showViewMoreButton && (
        <div className="mega-saver-view-more-container" style={{ position: 'relative', zIndex: 10 }}>
          {/* Simple direct anchor tag for maximum compatibility */}
          <a
            href="/mega_saver"
...
// Debug function to inspect metaobject data
function debugMetaobjects(metaobjects) {
  if (!metaobjects || !metaobjects.nodes || metaobjects.nodes.length === 0) {
    console.log('No metaobjects found');
    return;
  }

  console.log(`Found ${metaobjects.nodes.length} metaobjects`);

  // Inspect the first metaobject
  const firstItem = metaobjects.nodes[0];
  console.log('First metaobject:', {
    id: firstItem.id,
    handle: firstItem.handle,
    type: firstItem.type,
    fieldCount: firstItem.fields ? firstItem.fields.length : 0
  });

  // Log all fields of the first metaobject
  if (firstItem.fields && firstItem.fields.length > 0) {
    console.log('Fields of first metaobject:');
    firstItem.fields.forEach(field => {
      console.log(`Field: ${field.key}, Type: ${field.type}, Value: ${field.value ? field.value.substring(0, 50) : 'null'}, Has Reference: ${field.reference ? 'Yes' : 'No'}`);

      if (field.reference) {
        if (field.reference.image) {
          console.log(`  Image URL: ${field.reference.image.url}`);
        } else if (field.reference.id) {
          console.log(`  Reference ID: ${field.reference.id}`);
        }
      }
    });
  }
